...
class DES:
    ...
    def encrypt_text_cfb(self, text: str, key_hex: str, iv_hex = None, k: int = 8) -> str:
        import os
        if len(key_hex) != 16:
            raise ValueError("Ключ должен быть 16 hex-символов (64 бит)")
        key = BitString.from_bits(bin(int(key_hex, 16))[2:].zfill(64))
        ...
        for i in range(len(data)):
            ...
            shift_register = BitString.from_bits(shift_register.bits[k:] + cipher_k_bits.bits)

        cipher_hex = f"{int(cipher_bits, 2):0{len(cipher_bits) // 4}X}"

        return iv_hex.upper() + cipher_hex

    def decrypt_text_cfb(self, cipher_with_iv: str, key_hex: str, k: int = 8) -> str:
        if len(key_hex) != 16:
            raise ValueError("Ключ должен быть 16 hex-символов (64 бит)")
        key = BitString.from_bits(bin(int(key_hex, 16))[2:].zfill(64))
